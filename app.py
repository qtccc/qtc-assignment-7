# assignment7_starter_code/app.py
...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    Y = beta0 + beta1 * X + mu + error  # Code to generate Y

    # TODO 3: Fit a linear regression model to X and Y
    model = LinearRegression()  # Initialize the LinearRegression model
    # Fit the model to X and Y
    X_reshaped = X.reshape(-1,1) # Reshape X for mdoel fitting
    ...
    # Code to generate and save the scatter plot
    plt.figure()
    plt.scatter(X, Y, label="Data points")
    plt.plot(X, model.predict(X_reshaped), color="red", label="Regression line") # Plot the fitted line
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    ...
    for _ in range(S):
        ...
        X_sim = np.random.rand(N)  # to generate simulated X values
        error_sim = np.random.normal(loc=0, scale=np.sqrt(sigma2), size=N)
        Y_sim = beta0 + beta1 *X_sim + mu + error_sim  # to generate simulated Y values


        # TODO 7: Fit linear regression to simulated data and store slope and intercept
        X_sim_reshaped = X_sim.reshape(-1, 1)
        sim_model = LinearRegression()  # to fit the model
    ...
    intercept_more_extreme = sum(abs(i) > abs(intercept) for i in intercepts) / S  # Replace with code to calculate proportion of intercepts more extreme than observed
    intercept_extreme = sum(abs(i) > abs(intercept) for i in intercepts) / S
...
def hypothesis_test():
    ...
    plt.figure()
    plt.hist(simulated_stats, bins=30, color="blue", alpha=0.7, label="Simulated Statistics")
    plt.axvline(x=observed_stat, color="red", linestyle="--", label="Observed Statistic")
    plt.axvline(x=hypothesized_value, color="green", linestyle="-", label="Hypothesized Value")
...
def confidence_interval():
    ...
    if len(estimates) <= 1 or std_estimate == 0:
        # If we can't calculate a proper confidence interval, set bounds to the mean estimate
        ci_lower, ci_upper = mean_estimate, mean_estimate
    else:
        # Calculate the critical value for the t-distribution
        alpha = 1 - confidence_level  # E.g., for 95% confidence, alpha = 0.05
        t_critical = t.ppf(1 - alpha / 2, df=len(estimates) - 1)  # Two-tailed critical value

        # Calculate margin of error
        margin_of_error = t_critical * std_estimate
    ...
    # Plot mean estimate as a blue point
    ...
    # Plot the true parameter as a green line
